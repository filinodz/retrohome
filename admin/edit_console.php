...
<head>
    ...
    <title><?= __('admin_edit_console_title') ?> : <?= htmlspecialchars($console['name'] ?? 'Console') ?> - <?= SITE_NAME ?></title>
    ...
    <div class="admin-container mx-auto px-4 sm:px-6 lg:px-8 py-8">
         ...
         <header class="admin-header-internal mb-8">
             <a href="index.php" class="back-link-header" title="<?= __('back_caps') ?>">
                <i class="fas fa-arrow-left mr-2"></i>
                 <h1 class="form-title inline"><?= __('admin_edit_console_title') ?> : <?= htmlspecialchars($console['name'] ?? 'Console') ?></h1>
            </a>
        </header>
        ...
        <?php if ($console): // Affiche le formulaire seulement si $console a été chargée ?>
            <div class="admin-form-container animate__animated animate__fadeInUp">
                <form action="edit_console?id=<?= $console_id ?>" method="post" enctype="multipart/form-data" novalidate>
                     <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div class="form-group">
                            <label for="logo"><?= __('admin_replace_cover') ?> :</label>
                            <input type="file" id="logo" name="logo" accept="image/jpeg,image/png,image/gif,image/webp,image/svg+xml">
                             <?php if (!empty($console['logo'])): ?>
                                 ...
                                 <img src="..<?= htmlspecialchars($console['logo']) ?>?t=<?= time() ?>" alt="<?= __('admin_current_file') ?>" class="current-preview-image">
                            <?php else: ?>
                                <p class="current-file-info text-orange-400"><i class="fas fa-image"></i><?= __('admin_no_current_file') ?></p>
                            <?php endif; ?>
                        ...
                        <div class="form-group">
                            <label for="sort_order"><?= __('admin_sort_label') ?> :</label>
