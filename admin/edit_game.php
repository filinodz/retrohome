...
<head>
    ...
    <title><?= __('admin_edit_game_title') ?> : <?= htmlspecialchars($game['title'] ?? 'Jeu') ?> - <?= SITE_NAME ?></title>
    <link rel="icon" type="image/png" href="../public/img/logo_new.png">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <!-- CSS dependencies -->
    <link href="../public/vendor/tailwindcss/2.2.19/tailwind.min.css" rel="stylesheet">
    ...
    <link rel="stylesheet" href="public/css/admin_style.css?v=<?= @filemtime(__DIR__ . "/public/css/admin_style.css") ?>"> <!-- Inclure les styles admin -->
</head>
        ...
        <?php endif; ?>
         ...
         <?php if ($game): // Affiche le formulaire seulement si $game a été chargé ?>
            <div class="admin-form-container animate__animated animate__fadeInUp">
                <form action="edit_game?id=<?= $game_id ?>" method="post" enctype="multipart/form-data" novalidate>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div class="form-group">
                            ...
                            <select id="console_id" name="console_id" required>
                                <option value="" disabled><?= __('admin_choose_console') ?></option>
                        ...
                        <div class="form-group md:col-span-2 border-t border-border-color pt-6 mt-4">
                            ...
                            <?php endif; ?>
                             <p class="text-xs text-text-secondary mt-1"><?= __('admin_optional') ?></p>
                        </div>

                        <div class="form-group">
                            <label for="cover"><?= __('admin_replace_cover') ?> :</label>
                        ...
                        <div class="form-group">
                            <label for="preview"><?= __('admin_replace_video') ?> :</label>
                            <input type="file" id="preview" name="preview" accept="video/mp4,video/webm">
                             <?php if (!empty($game['preview'])): ?>
                                <p class="current-file-info"><i class="fas fa-film"></i><?= __('admin_current_file') ?> <?= htmlspecialchars(basename($game['preview'])) ?></p>
                             <?php else: ?>
                                <p class="current-file-info"><i class="fas fa-video-slash"></i><?= __('admin_no_current_file') ?></p>
                            <?php endif; ?>
                    ...
                    <div class="form-actions">
                        <a href="index.php" class="form-button cancel-button">
                             <i class="fas fa-times mr-2"></i><?= __('admin_cancel') ?>
                        </a>
                        <button type="submit" class="form-button submit-button">
                            <i class="fas fa-save mr-2"></i><?= __('admin_save_mods') ?>
                        </button>
                    </div>
                </form>
            </div>
