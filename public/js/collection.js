document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadCollectionDetails() {
        ...
        try {
            const response = await fetch(`${SITE_URL}/api.php?action=getCollection&collection_id=${collectionId}`);
            if (!response.ok) {
    ...
    async function loadCollectionGames() {
        ...
        try {
            const response = await fetch(`${SITE_URL}/api.php?action=getCollectionGames&collection_id=${collectionId}`);
    ...
    async function loadConsolesForFilter() {
        try {
            const response = await fetch(`${SITE_URL}/api.php?action=getConsoles`);
    ...
    function hideGameModal() {
        ...
        // Simple fallback to ensure it closes
        const close = () => {
        ...
        elements.modalContent.addEventListener('animationend', close, { once: true });
        // Emergency fallback after animation time
        setTimeout(() => {
            if (!elements.gameModal.classList.contains('hidden')) close();
        }, 500);
    }
    ...
    async function loadModalGamesList() {
        ...
        try {
            ...
            const response = await fetch(`${SITE_URL}/api.php?action=getGames`); // Pas de filtre ici
    ...
    async function toggleGameInCollection(gameId, buttonElement) {
        ...
        try {
            ...
            if (isAdding) {
                ...
                // AJOUT OPTIMISTE : Ajouter la carte à la grille principale sans recharger
                const gameData = allLibraryGames.find(g => g.id == gameId);
                if (gameData) {
                    // Supprimer le message "vide" si présent
                    const emptyMsg = elements.gamesContainer.querySelector('.loading-placeholder.italic');
                    if (emptyMsg) emptyMsg.remove();

                    const newCard = createCollectionGameCard(gameData);
                    elements.gamesContainer.appendChild(newCard);
                    filterCollectionGamesUI(); // Appliquer filtres
                }
            } else {
                ...
                // SUPPRESSION OPTIMISTE : Supprimer dynamiquement la carte de la grille principale
                const cardToRemove = elements.gamesContainer.querySelector(`.collection-game-card[data-game-id="${gameId}"]`);
                if (cardToRemove) {
                    const finalize = () => {
                        cardToRemove.remove();
                        // Vérifier si la collection est vide après suppression animation
                        if (collectionGameIds.size === 0) {
                            elements.gamesContainer.innerHTML = '<p class="loading-placeholder col-span-full text-text-secondary italic">Cette collection est vide. Ajoutez des jeux !</p>';
                        }
                    };
                    cardToRemove.addEventListener('animationend', finalize, { once: true });
                    cardToRemove.classList.add('animate__animated', 'animate__fadeOut');
                    // Fallback si l'animation ne se déclenche pas
                    setTimeout(() => { if (cardToRemove.parentNode) finalize(); }, 600);
                }
            }
    ...
    async function removeGameFromCollection(collectionId, gameId, cardElement) {
        ...
        cardElement.addEventListener('animationend', finalizeRemoval, { once: true });

        // Démarrer l'animation (Sortie)
        cardElement.classList.add('animate__animated', 'animate__fadeOut');

        try {
            const response = await fetch(`${SITE_URL}/api.php?action=removeGameFromCollection`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: `collection_id=${collectionId}&game_id=${gameId}`
            });
            ...
            // Succès : Supprimer définitivement de l'état
            collectionGameIds.delete(gameId);

            // Si l'animation est déjà finie, l'élément est déjà supprimé.
            // Si elle ne s'est pas déclenchée pour une raison X, on force la suppression après un petit délai
            setTimeout(() => {
                if (cardElement.parentNode) finalizeRemoval();
            }, 600); // Durée légèrement supérieure à l'animation

        } catch (error) {
            console.error("Erreur lors de la suppression du jeu:", error);
            alert(`Erreur suppression: ${error.message}`);

            // ANNULATION : Restaurer la carte
            cardElement.removeEventListener('animationend', finalizeRemoval);
            cardElement.classList.remove('animate__fadeOut');
    ...
    function setViewMode(mode) {
        ...
        if (mode === 'grid') {
            ...
            elements.gamesContainer?.classList.remove('view-list');
        } else {
            listBtn?.classList.add('active');
            gridBtn?.classList.remove('active');
            elements.gamesContainer?.classList.add('view-list');
        }
    }
